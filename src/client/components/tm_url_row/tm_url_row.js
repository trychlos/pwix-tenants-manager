/*
 * /imports/client/components/tm_url_row/tm_url_row.js
 *
 * Manage a contact email address, maybe empty but have at least an id.
 *
 * Parms (when editing the urls):
 ...
 *
 * Parms (when showing all the urls):
 * - edit: false
...
Template.tm_url_row.onCreated( function(){
    const self = this;
    ...
    self.TM = {
        ...
        removeById( id ){
            ...
            if( found !== -1 ){
                ...
                const trs = self.$( 'tr.tm-url-row' ).closest( 'table' ).children( 'tr' );
...
Template.tm_url_row.onRendered( function(){
    const self = this;

    // initialize the Checker for this panel as soon as we get the parent Checker
    // nb: this same template is used both for editing the URLS and for displaying them
...
Template.tm_url_row.helpers({
    ...
    readonly(){
        return ( this.edit === false ) ? 'readonly' : '';
    }
});

Template.tm_url_row.events({
    'click .tm-url-row .js-minus'( event, instance ){
        const id = this.it._id;
        instance.TM.removeById( id );
    },
});

Template.tm_url_row.onDestroyed( function(){
